...
void *pollhandler(void *arg)
{
    preqests p_reqests = &((preqests)arg)[0];
    int curr_fd = p_reqests->_fd;
    int nbytes;
    int index;
    int fd_pool;
    for (;;)
    {
        bzero(buf, MAXLEN);
        if ((nbytes = recv(curr_fd, buf, sizeof(buf), 0)) == -1) // error
        {
        ...
        {
            index = 0;
            while (index < fd_count + 1)
            {
                fd_pool = pfds[index].fd;
                if (fd_pool != curr_fd && fd_pool != listener)
                {
                    send(fd_pool, buf, nbytes, 0);
                }
                index++;
            }
        }
